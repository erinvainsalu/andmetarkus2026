...
from budget_table bt 
	left join sales_rep_table srt on bt.sales_rep_id = srt.sales_rep_id 
...
select *
from sales_rep_table srt 
	join budget_table bt on srt.sales_rep_id = bt.sales_rep_id 

-- 10.5. Näita kõiki ridu eelarve tabelist ja kõiki ridu müügiesindaja tabelist.
select *
from budget_table bt 
	full outer join sales_rep_table srt on bt.sales_rep_id = srt.sales_rep_id 
...
select srt.sales_rep_id, bt.sales_rep_id  
from sales_rep_table srt 
	full outer join budget_table bt on srt.sales_rep_id = bt.sales_rep_id 
...
select st.*
from sales_table st 
	join budget_table bt on st.sales_rep_id = bt.sales_rep_id 
	join sales_rep_table srt on st.sales_rep_id = srt.sales_rep_id 
